...
currentNumber = 8
isCurrentNumber = y == currentNumber
y = y * isCurrentNumber
y = y / currentNumber
...
for o in range(K_ITER):
	...
	ALFA = (1 / (1 + 0.01*o))*ALFA_ZERO  # learning rate decay
	t = 0
	...
	while t < 34000:
		...
		J = (1/m_mini) * (- np.sum(((y_mini*np.log(a3)) + ((1 - y_mini)*np.log(1-a3)))))
...
J_dev = (1/8000) * (- np.sum(((y_dev*np.log(a3)) + ((1 - y_dev)*np.log(1-a3)))))
